...
std::unordered_set<std::string> harmful_domains;
...
void load_blocklist(const char* filename) {
    ...
    while (std::getline(in, domain)) {
        domain = trim(domain);
        if (!domain.empty()) {
            harmful_domains.insert(domain);
        }
    }
    printf("🧾 Loaded %zu harmful domains\n", harmful_domains.size());
}
...
bool isHarmfulSite(unsigned char *payload, int payload_len) {
    ...
    if (host) {
        ...
        if (harmful_domains.count(domain) > 0) {
            printf("차단 도메인입니다: %s\n", domain);
            return true;
        }
    }
              ...
              struct nfq_data *nfa, void *data)
{
    ...
    if (payload_len > 0) {
        if(isHarmfulSite(payload, payload_len))
            verdict = NF_DROP;
    }

    auto end = high_resolution_clock::now();
    auto duration = duration_cast<microseconds>(end - start);
    printf("패킷 처리 시간: %lldμs\n", (long long)duration.count());

    return nfq_set_verdict(qh, print_pkt(nfa), verdict, 0, NULL);
}
